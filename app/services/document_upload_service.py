...
def upload_document(db: Session, file: UploadFile):
    ...
    document = create_document(db=db, filename=file.filename, file_type="pdf", file_path=file_path, document_hash=document_hash, status="processing")

    try:
        chunk_count = ingest_document(db=db, document_id=document.id, filename=document.filename, file_path=document.file_path)
