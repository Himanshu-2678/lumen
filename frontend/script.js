...
async function submitQuestion() {
    ...
    try {
        ...
        answerText.innerHTML = `
            <span class="thinking">
                Finding relevant evidence<span class="dots"></span>
            </span>
        `;
        ...
        answerText.innerHTML = `
            <span class="thinking">
                Generating answer<span class="dots"></span>
            </span>
        `;
        ...
        if (data.confidence) {
            const score = Math.round(
                data.confidence.confidence_score * 100
            );
    ...
    documents.forEach(doc => {
        ...
        li.innerHTML = `
            <div class="docrow__left">
                <span class="docrow__icon">
                    ${docIconSvg()}
                </span>

                <div>
                    <span class="docrow__name">
                        ${doc.filename}
                    </span>
...
docList.addEventListener(
    "click",
    async event => {
        const button =
            event.target.closest(".docrow__del");
...
openKb.addEventListener(
    "click",
    openModal
);

closeKb.addEventListener(
    "click",
    closeModal
);

kbBackdrop.addEventListener(
    "click",
    closeModal
);
